...
Every illustration is a rendered PNG on its own solid cream ground, a "plate", exported at exactly
2x its CSS size and given its corners by the shared `.plate` class. `docs/design-direction.md` still
governs the interface; this file governs the images.

## Theme

Trees and plants. Clean 3D computer graphics: sharp, simple, a product-render look under soft studio
light. VirtusLab greens, warm wood, on a cream `#f4efe6` ground. No text, logos, people, animals or
mascots.
...
| `public/og.png` | 1200×630 | share card | the hero plate plus the title set in Montserrat |
| `public/favicon.svg`, `favicon-32.png`, `apple-touch-icon.png` (180) | — | favicons | the logo part of the hero on a cream tile |
...
subject's bounding box on the cream ground. `og` composes the hero plate with the title in Montserrat
and `favicons` cuts the logo part of the hero onto a cream tile (both need the two variable fonts in
`fonts/`, see the script). Renders come out at 1024 or 1536 on the long side.
...
- Solid cream ground, edge to edge.
- No text.
