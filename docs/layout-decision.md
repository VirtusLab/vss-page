...
Global rules: one breakpoint at 720px. No client-side JavaScript except the star-count refresh,
the picker's current-chapter mark and the agent prompt's copy button; the page must be complete
without any of them. Every section and every component carries an explicit `id` in the YAML, used
verbatim as its anchor. Content comes from `content/components.yaml`; snippets from `.scala` files
compiled in CI.
...
- Under the hero and the divider that closes it, still within the first scroll on desktop. The block
  ...
  benefits band follows, so the order is hero → divider → switcher → agent prompt → promo →
